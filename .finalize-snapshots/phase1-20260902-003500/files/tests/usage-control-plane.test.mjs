...
test("optional usage snapshots degrade intentionally without a development error overlay", () => {
  const usage = read("src/lib/usage/reserve-usage.ts");
  const settings = read("src/app/(protected)/settings/page.tsx");

  assert.match(settings, /activeTab === "usage" && business/);
  assert.match(usage, /mode === "enforce"[\s\S]*console\.error\("Usage snapshot error"/);
  assert.match(usage, /console\.warn\("Usage snapshot unavailable in observe mode"/);
  assert.match(usage, /return unavailableSnapshot\(planId\)/);
});
