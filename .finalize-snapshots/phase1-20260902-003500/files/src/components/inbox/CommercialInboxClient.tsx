...
function matchesQueueFilter(signal: CommercialSignal, filter: QueueFilter) {
  return filter === "all"
    || (filter === "review" && ["new", "ready_for_review", "postponed"].includes(signal.reviewStatus) && signal.status !== "archived")
    || (filter === "linked" && Boolean(signal.matchedOrganizationId || signal.matchedContactId || signal.detectedFromOpportunityId) && signal.status !== "converted" && signal.status !== "archived")
    || (filter === "converted" && signal.status === "converted")
    || (filter === "archived" && signal.status === "archived");
}
...
export function CommercialInboxClient({
  ...
  const filteredSignals = useMemo(() => signals.filter((signal) => {
    ...
    const queueMatch = matchesQueueFilter(signal, queueFilter);
  ...
  const queueFilterOptions = queueFilters.map((filter) => ({
    ...filter,
    count: signals.filter((signal) => matchesQueueFilter(signal, filter.id)).length,
  }));
  const profileNameById = useMemo(
    () => new Map(assignableProfiles.map((profile) => [profile.id, profile.fullName])),
    [assignableProfiles],
  );
  ...
  function runAction(
    action: () => Promise<{ ok: boolean; outcome?: "applied" | "already_applied" | "conflict"; message?: string; signal?: CommercialSignal; fallbackUsed?: boolean; opportunityId?: string; alreadyConverted?: boolean }>,
    ...
    startTransition(async () => {
      ...
      const message = result.fallbackUsed
        ...
        : result.outcome === "already_applied"
          ? "Decizia era deja aplicată; acțiunea curentă nu a repetat conversia."
          : successMessage;
      setNotice(message);
      showToast({
  ...
  function decide(decision: "dismissed" | "duplicate" | "postponed") {
    if (!selectedSignal) return;
    runAction(() => decision === "dismissed"
      ? rejectCommercialSignal(selectedSignal.id, selectedSignal.updatedAt ?? "", decisionReason)
      : setCommercialSignalReviewDecision(selectedSignal.id, decision, decisionReason, postponeUntil),
    ...
    <div className="app-section-stack">
      ...
      <section aria-label="Comenzi Inbox Comercial" className="border-y border-[rgb(var(--border))]">
        <div className="flex flex-wrap items-center justify-between gap-3 py-2">
          <SegmentedFilter label="Starea semnalelor" options={queueFilterOptions} value={queueFilter} onChange={setQueueFilter} />
      ...
      <div className="product-work-surface grid min-w-0 overflow-hidden lg:max-h-[min(680px,calc(100vh-16rem))] lg:grid-cols-[minmax(20rem,0.78fr)_minmax(0,1.45fr)]">
        <section className="min-w-0 lg:overflow-y-auto lg:overscroll-contain lg:border-r" aria-labelledby="signal-list-title">
          ...
          <ul aria-label="Coada de semnale comerciale" className="divide-y divide-[rgb(var(--border))]">
            {filteredSignals.map((signal) => (
              <li key={signal.id}>
              <button type="button" onClick={() => selectSignal(signal)} aria-pressed={selectedId === signal.id} aria-controls="signal-review-panel" className={`focus-ring group relative grid w-full gap-2 border-l-2 px-3 py-3 text-left transition-colors ${selectedId === signal.id ? "border-l-[rgb(var(--interaction))] bg-[rgb(var(--interaction-tint)/0.62)]" : "border-l-transparent hover:bg-[rgb(var(--surface-subtle))]"}`}>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 text-[0.6875rem] text-[rgb(var(--text-faint))]">
                  <span>{reviewLabels[signal.reviewStatus]} · {profileNameById.get(signal.assignedToProfileId ?? signal.suggestedOwnerProfileId ?? "") ?? "Fără responsabil"}</span>
            ...
            {filteredSignals.length === 0 ? <li>{signals.length === 0 ? (
            ...
            ) : <EmptyState title="Niciun rezultat pentru filtrele curente" description="Modifică filtrele sau căutarea pentru a vedea alte semnale." />}</li> : null}
          </ul>
        </section>

        <section id="signal-review-panel" className="min-w-0 scroll-mt-24 border-t border-[rgb(var(--border))] lg:overflow-y-auto lg:overscroll-contain lg:border-t-0" aria-labelledby="signal-review-title">
          ...
          {selectedSignal ? (
            <div className="grid gap-5 p-4">
              <div className="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-[rgb(var(--border-subtle))] pb-3 text-xs">
                <span className="font-semibold">{reviewLabels[selectedSignal.reviewStatus]}</span>
                ...
                {selectedSignal.analysisMode ? <span className="text-[rgb(var(--text-muted))]">Triere asistată · reguli vizibile</span> : null}
                {(selectedSignal.events ?? []).some((event) => event.eventType === "analysis_review_edited") ? <span className="text-[rgb(var(--text-muted))]">Editat de utilizator</span> : null}
                <span className="font-medium text-[rgb(var(--text-muted))]">Netrimis automat</span>
              ...
              <section aria-label="Acțiunea comercială sigură" className="product-grouping-surface grid gap-3 p-3.5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                <div>
                  <p className="text-xs font-semibold text-[rgb(var(--foreground))]">Următoarea decizie sigură</p>
                  <p className="mt-1 text-xs leading-5 text-[rgb(var(--text-muted))]">{reviewForm.opportunityId
                    ? "La aprobare, oportunitatea selectată primește acțiunea internă, termenul și responsabilul revizuite. Nu se trimite niciun mesaj."
                    : "La aprobare, ReveNew creează o oportunitate și o acțiune internă din datele revizuite. Nu se trimite niciun mesaj."}</p>
                </div>
                <div className="flex flex-wrap gap-2 sm:justify-end">
                  <Button variant="secondary" onClick={saveReviewFields} disabled={isPending}>Salvează contextul</Button>
