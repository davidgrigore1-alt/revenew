...
export async function getCommercialWorkflowWorkspace() { const actor = await currentWorkflowActor(); const client = admin(); const [{ data: workflows }, { data: runs }, { data: opportunities }] = await Promise.all([client.from("commercial_workflows").select("*").eq("business_id", actor.businessId).order("updated_at", { ascending: false }).limit(50), client.from("commercial_workflow_runs").select("id,workflow_id,trigger_type,target_id,status,guard_decision,guard_reason,failure_category,is_test_run,created_at").eq("business_id", actor.businessId).order("created_at", { ascending: false }).limit(50), client.from("opportunities").select("id,title").eq("business_id", actor.businessId).order("updated_at", { ascending: false }).limit(100)]); return { workflows: (workflows ?? []).map((row) => rowDefinition(row as WorkflowRow)), runs: runs ?? [], opportunities: opportunities ?? [] }; }
