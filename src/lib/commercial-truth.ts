...
export function assembleCommercialTruth(input:{
 ...
 const currentState=buildOpportunityCommercialState(opportunity,{businessId:input.businessId,now:new Date(evaluatedAt),linkedSignals:input.linkedSignals});
 ...
 const add=(type:ClaimType,label:string,value:string,evidence:EvidenceReference[],extra:Partial<TruthClaim>={})=>{
  ...
  const claim:TruthClaim={id:opportunity.id+":"+type+":"+claims.length,type,kind:"fact",opportunityId:opportunity.id,label,value,
   state:"confirmed",freshness:extra.derivation==="explicit_source_field"?freshness(observedAt,now):"current",observedAt,derivation:"structured_record",evidence,...extra};
 ...
 const owner=add("owner","Responsabil",currentState.ownership.ownerName??(currentState.ownership.ownerProfileId?"Responsabil atribuit":"Lipsește"),[record]);
 ...
 const currentAction=currentState.nextAction?pending.find(action=>action.id===currentState.nextAction?.id):undefined;
 const future=pending.find(action=>Number.isFinite(Date.parse(action.dueDate))&&Date.parse(action.dueDate)>=now);
 const next=add("next_step","Următor pas",currentAction?.title??(!active?"Nu este necesar (închis)":opportunity.actions.length>=200?"Neverificat în lista disponibilă":"Lipsește"),currentAction?[metadataEvidence({sourceType:"action",sourceId:currentAction.id,title:currentAction.title,occurredAt:currentAction.updatedAt??currentAction.createdAt??null,entityHref:route+"?tab=schedule"})]:[record]);
 if(currentState.nextAction?.dueAt)add("next_step_due_at","Termen acțiune",currentState.nextAction.dueAt.slice(0,10),next!.evidence,{date:currentState.nextAction.dueAt});
 else if(active&&currentAction&&next)issue("missing-action-date","missing","Confirmă termenul acțiunii","Acțiunea există, dar termenul curent nu este confirmat.","Pasul nu are un termen verificabil.","Confirmă termenul prin fluxul existent.",[next],85);
 ...
 if(currentState.nextAction?.overdue&&next)issue("overdue-next","interpretation","Acțiune restantă","Termenul acțiunii curente este depășit.","Următorul pas necesită o decizie.","Revizuiește termenul sau confirmă finalizarea.",[next],95);
 if(currentState.approval.state==="pending"){
  const approval=add("approval_requirement","Decizie comercială","Aprobare în așteptare",[record]);
  if(approval)issue("pending-approval","interpretation","Decizie comercială în așteptare","Decizia de aprobare nu este încă înregistrată.","Progresul necesită control uman.","Verifică aprobarea.",[approval],90);
 }
