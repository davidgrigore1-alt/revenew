...
export async function commercialTruthAnswer(request:CopilotRequest):Promise<CopilotAnswer>{
 ...
 try{
  if(/^\/dashboard\?view=(?:review|executive)(?:&|$)/.test(request.context.route)
   && !/contrazic|neconcord|asocierea document|difer.*(?:crm|dovez|ofert)|document.*(?:deadline|termen)/i.test(request.question)){
   const { revenueCommandAnswer }=await import("@/lib/ai/revenue-command-answer");
   return revenueCommandAnswer(request);
  }
