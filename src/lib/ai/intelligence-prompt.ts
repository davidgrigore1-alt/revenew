...
export function buildIntelligencePrompt(request: CopilotRequest, evidence: CopilotEvidence[], limits: string[], repair: string | null = null) {
  ...
  for (const item of evidence) {
    ...
    const candidate: CopilotEvidence = { sourceId: alias, label: item.label.slice(0, 140), fact: item.fact.slice(0, 800), sourceType: item.sourceType, claimType: item.claimType, observedAt: item.observedAt, route: null };
