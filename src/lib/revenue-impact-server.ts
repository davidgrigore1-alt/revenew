...
export async function getRevenueImpact(input:{range?:string;from?:string;to?:string;opportunityId?:string;opportunityIds?:string[];includeOutsidePeriod?:boolean}={}){
 ...
 if(input.opportunityIds&&(!input.opportunityIds.length||input.opportunityIds.length>80||input.opportunityIds.some(id=>!uuidPattern.test(id))))return empty;
 ...
 if(input.opportunityIds)query=query.in("opportunity_id",input.opportunityIds);
 ...
 const visibleProofs=input.opportunityId||input.includeOutsidePeriod?proofs:proofs.filter(p=>p.events.some(e=>e.created_at>=period.from&&e.created_at<period.to));
 const profiles=visibleProofs.length?await client.rpc("business_assignable_profiles",{target_business_id:current.business.id}):{data:[]};
 const actorNames:Record<string,string>=Object.fromEntries((profiles.data??[]).map((p:{profile_id:string;full_name:string})=>[p.profile_id,p.full_name]));
 return {actorNames,proofs:visibleProofs,summary:limited?summarizeImpact([],period.from,period.to):summarizeImpact(proofs,period.from,period.to),period,limited,available:true,
