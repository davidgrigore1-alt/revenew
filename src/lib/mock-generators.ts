...
Din profilul oportunitatii, intelegem ca sunt importante disponibilitatea, viteza de raspuns si o solutie comerciala clara pentru zona ${opportunity.city}/${opportunity.county}. ${business.name} poate sustine acest tip de situatie prin ${serviceLine(business)}, cu o abordare adaptata duratei, volumului si conditiilor operationale.
...
Daca este relevant, propun o discutie de 10-15 minute pentru a confirma contextul, disponibilitatea si pasii urmatori. Imi puteti indica, va rog, o fereastra potrivita sau persoana responsabila pentru aceasta solicitare?
...
1. Deschidere
Buna ziua, sunt de la ${business.name}. Va contactez in legatura cu oportunitatea "${opportunity.title}". Am vrut sa verific rapid daca subiectul este inca activ si cine coordoneaza discutia comerciala.
...
- Exista un volum estimativ, o perioada de utilizare sau un buget orientativ?
...
Estimarea finala depinde de cerintele confirmate, calendar, volum si conditii operationale.

Pasi urmatori
1. Confirmarea contactului si a persoanei decidente.
...
- Confirma nevoia reala si contextul comercial.
- Verifica termenul limita: ${formatDate(opportunity.deadline)}.
- Confirma locatia principala: ${opportunity.city}/${opportunity.county}.
- Verifica disponibilitatea pentru serviciile relevante: ${serviceLine(business)}.
- Pregateste intervalul orientativ: ${formatCurrency(opportunity.estimatedValueLow)} - ${formatCurrency(opportunity.estimatedValueHigh)}.
- Stabileste urmatorul pas: email, apel, oferta sau clarificare.
- Programeaza follow-up la 48 de ore dupa primul contact.
${procurement ? "- Verifica documentele obligatorii, criteriile de calificare, garantiile si termenii procedurii.\n" : ""}${grant ? "- Verifica eligibilitatea, documentele financiare si conditiile programului de finantare.\n" : ""}- Noteaza concluzia in statusul oportunitatii.`;
}
...
Daca subiectul este inca activ, putem discuta 10-15 minute pentru a confirma nevoia si pasii urmatori. Alternativ, ne puteti trimite cateva detalii suplimentare pentru a reveni cu o varianta orientativa.
