...
const definitions = {
 ...
 next_action_undated: { kind: "next_action_missing", label: "Următorul pas necesită un termen", done: "Acțiunea existentă primește un termen viitor confirmat." },
...
export function projectCommercialReview(model: RevenueCommand) {
 return model.decisions.map(decision => {
  ...
  return {
   ...decision, action:{...decision.action,href:evidenceHref(decision.action.href)??decision.action.href}, kind: definition?.kind ?? "stalled_execution",
