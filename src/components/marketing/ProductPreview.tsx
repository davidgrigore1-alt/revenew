  ...
  return (
    <figure className="mx-auto w-full max-w-[1240px]" aria-labelledby="product-preview-caption">
      <div className="relative isolate" data-marketing-product-frame>
        ...
        <div aria-hidden="true" className="pointer-events-none absolute -left-16 top-[9.25rem] z-20 hidden w-52 rounded-xl border border-emerald-200/90 bg-white/95 p-4 shadow-[0_18px_55px_rgba(15,23,42,0.16)] backdrop-blur-sm 2xl:block">
          <p className="text-[0.63rem] font-bold uppercase tracking-[0.14em] text-emerald-700">Dovadă</p>
          <p className="mt-2 text-xs font-semibold leading-5 text-slate-900">Termenul următoarei acțiuni a fost depășit.</p>
          <div className="mt-3 flex items-center gap-2 border-t border-slate-200 pt-3 text-[0.67rem] text-slate-500"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />Sursă verificabilă</div>
        </div>

        <div aria-hidden="true" className="pointer-events-none absolute -right-16 bottom-[7rem] z-20 hidden w-56 rounded-xl border border-emerald-200/90 bg-white/95 p-4 shadow-[0_18px_55px_rgba(15,23,42,0.16)] backdrop-blur-sm 2xl:block">
          <p className="text-[0.63rem] font-bold uppercase tracking-[0.14em] text-emerald-700">Control uman</p>
          <p className="mt-2 text-xs font-semibold leading-5 text-slate-900">Aprobarea rămâne la echipă, înainte de orice acțiune.</p>
          <div className="mt-3 flex items-center gap-2 border-t border-slate-200 pt-3 text-[0.67rem] text-slate-500"><ShieldCheckIcon className="h-3.5 w-3.5 text-emerald-700" />Nicio trimitere automată</div>
        </div>
          ...
          <div className="grid min-h-[31rem] bg-white md:grid-cols-[11.5rem_minmax(0,1fr)] lg:min-h-[37rem]">
            ...
            <section className="min-w-0 text-slate-950" aria-label="Exemplu de suprafață de lucru ReveNew">
              <header className="flex min-h-12 items-center justify-between gap-3 border-b border-slate-200 px-4 sm:px-5">
                ...
                <span className="inline-flex items-center gap-1.5 text-[0.7rem] text-slate-600"><ShieldCheckIcon className="h-3.5 w-3.5 text-teal-700" aria-hidden="true" />Control uman activ</span>
              </header>
              ...
              <div className="grid lg:grid-cols-[minmax(0,1fr)_17.5rem]">
                  ...
                  <div className="mx-auto max-w-[650px]">
                    ...
                    <div className="mt-4 rounded-xl border border-slate-300 bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.06)]">
                      ...
                      <div className="mt-10 flex flex-wrap items-end justify-between gap-3 sm:mt-12">
                        <span className="inline-flex items-center gap-1.5 text-[0.69rem] text-slate-600"><ShieldCheckIcon className="h-3.5 w-3.5 text-teal-700" aria-hidden="true" />Doar informații autorizate</span>
                        <span className="rounded-md bg-teal-700 px-3 py-1.5 text-[0.72rem] font-semibold text-white">Verifică</span>
                      </div>
                    </div>
                    ...
                    <section className="mt-7" aria-labelledby="preview-attention-title">
                      <div className="flex items-end justify-between gap-4 border-b border-slate-200 pb-2.5">
                        <div><p className="text-[0.65rem] font-semibold uppercase tracking-[0.12em] text-teal-700">Astăzi</p><h3 id="preview-attention-title" className="mt-1 text-xs font-semibold">Necesită atenție</h3></div>
                        <span className="text-[0.67rem] text-slate-500">Prioritate explicabilă</span>
                      </div>
                      <div className="divide-y divide-slate-200">
                        {attentionRows.map((row, index) => (
                          <article key={row.title} className={`grid gap-2 py-3 ${index === 0 ? "bg-teal-50/60" : ""} sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:px-2`}>
                ...
                <aside className="bg-slate-50/75 p-4 sm:p-5" aria-labelledby="preview-context-title">
                  <p className="text-[0.65rem] font-semibold uppercase tracking-[0.13em] text-teal-700">Semnal selectat</p>
                  ...
                  <div className="mt-4 rounded-lg border border-teal-200 bg-teal-50 p-3">
                    <p className="inline-flex items-center gap-1.5 text-[0.7rem] font-semibold text-teal-900"><DocumentCheckIcon className="h-3.5 w-3.5" aria-hidden="true" />Aprobare umană</p>
                    <p className="mt-1.5 text-[0.68rem] leading-5 text-teal-900/75">Recomandarea poate fi verificată, editată sau respinsă înainte de orice acțiune.</p>
                  </div>
