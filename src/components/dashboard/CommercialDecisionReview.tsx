...
const agendaGrid = "grid border-l-2 grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_6rem_6rem_11.5rem] xl:items-center";
...
export function CommercialDecisionReview({model}:{model:RevenueCommandModel}) {
 ...
 return <section aria-labelledby="commercial-review-title" className="commercial-decision-review py-6">
  ...
  {phase==="after"?<section aria-live="polite" className="border-b border-[rgb(var(--border))] py-6">
   ...
   <div className="mt-4"><ActionToolbar label="După revizuire"><Button size="small" href="/dashboard">Revino la Control Center</Button><Button size="small" variant="secondary" onClick={()=>{setPhase("before");setVisited([]);}}>Vezi agenda curentă</Button></ActionToolbar></div>
   ...
   <section aria-labelledby="decision-agenda-title" className={"min-w-0 " + (phase==="during"?patterns.reviewPane:"")}>
    ...
    {agenda.length?<ul className="commercial-review-agenda divide-y divide-[rgb(var(--border))]">{agenda.map(item=><li key={item.id} className={agendaGrid+" commercial-review-agenda-row border-l-[rgb(var(--border-strong)/0.55)] px-3 py-3 transition-colors hover:border-l-[rgb(var(--border-strong))] "+(phase==="during"&&item.id===selected?.id?"commercial-review-current border-l-[rgb(var(--primary))] bg-[rgb(var(--surface-elevated))]":"")}>
     <button type="button" onClick={()=>select(item.id)} aria-pressed={phase==="during"&&item.id===selected?.id} aria-controls="commercial-review-detail" className="focus-ring min-w-0 text-left">
      ...
      <span className="mt-2 block"><CaseReadiness owner={Boolean(item.state.ownership.ownerProfileId)} action={Boolean(item.state.nextAction)} dated={Boolean(item.state.nextAction?.dueAt)} overdue={item.state.nextAction?.overdue}/></span>
   ...
   {phase==="during"?<aside id="commercial-review-detail" aria-label="Detaliul deciziei" className={"min-w-0 border-t border-[rgb(var(--border))] pt-4 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0 " + patterns.reviewPane}>
    {selected?<><h2 ref={detailHeading} tabIndex={-1} className="focus-ring break-words text-base font-semibold">{selected.state.title}</h2>
     ...
     <div className="mt-4"><ActionToolbar label="Navigare în revizuire">
  ...
  {phase!=="after"?<div className="mt-6 grid gap-6 border-t border-[rgb(var(--border))] pt-4 lg:grid-cols-2">
   <section><h2 className="text-sm font-semibold">{model.checkpoint?"De la ultima revizuire":"Schimbări în interval"}</h2>
    {model.historyClipped?<p className={"mt-1 "+muted}>Ultima revizuire este anterioară intervalului. Sunt afișate numai schimbările din {model.period.label.toLowerCase()}.</p>:null}
