...
const typeLabels: Record<Opportunity["type"], string> = {
  public_procurement: "Achizitie publica",
  b2b_lead: "Lead B2B",
  grant: "Grant",
  partnership: "Parteneriat",
  invoice_followup: "Factura",
  contract_renewal: "Reinnoire",
...
export function OpportunityCard({ opportunity }: { opportunity: Opportunity }) {
  return (
    <Link
      ...
      className="block rounded-xl border border-white/10 bg-white/[0.045] p-5 transition hover:border-mint-400/35 hover:bg-white/[0.07] focus:outline-none focus:ring-2 focus:ring-mint-400/40"
    >
      ...
      <h2 className="mt-4 text-lg font-semibold leading-7 text-white">{opportunity.title}</h2>
      <p className="mt-2 line-clamp-2 text-sm leading-6 text-zinc-400">{opportunity.summary}</p>
