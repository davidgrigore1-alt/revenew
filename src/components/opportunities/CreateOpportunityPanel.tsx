...
export function CreateOpportunityPanel({ organizations, assignableProfiles = [], initialOpen = false }: { organizations: CrmOrganization[]; assignableProfiles?: Array<{ id: string; fullName: string }>; initialOpen?: boolean }) {
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/45" role="dialog" aria-modal="true" aria-label="Adaugă oportunitate" onKeyDown={(event) => { if (event.key === "Escape") setOpen(false); }}>
          ...
          <div ref={panelRef} className="relative h-full w-full max-w-xl overflow-y-auto border-l border-[rgb(var(--border))] bg-[rgb(var(--surface))] p-5 shadow-2xl">
            ...
            <form action={submit} className="mt-6 grid gap-4">
              ...
              <label className="grid gap-2 text-sm font-semibold">Responsabil<span className="text-[0.6875rem] font-normal text-[rgb(var(--text-muted))]">Opțional</span><Select name="ownerProfileId" defaultValue="" className="h-11 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--background))] px-3"><option value="">Neatribuit</option>{assignableProfiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.fullName}</option>)}</Select></label>
