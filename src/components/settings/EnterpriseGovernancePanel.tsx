...
export function EnterpriseGovernancePanel({ snapshot }: { snapshot: any }) {
  ...
  return <div className="grid gap-6">
    {notice ? <p role="status" className="rounded-lg border border-emerald-400/30 bg-emerald-400/10 p-3 text-sm">{notice}</p> : null}
    {error ? <p role="alert" className="rounded-lg border border-rose-400/30 bg-rose-400/10 p-3 text-sm">{error}</p> : null}

    <section className="grid gap-4 rounded-xl border border-[rgb(var(--border))] p-5">
      ...
      {testLink ? <div className="rounded-lg border border-amber-400/30 bg-amber-400/10 p-4 text-sm"><strong>Link de acceptare — mod test</strong><p className="mt-1 break-all">{testLink}</p><p className="mt-1 text-xs">Vizibil numai administratorului care a creat invitația; tokenul nu este stocat în baza de date.</p></div> : null}
