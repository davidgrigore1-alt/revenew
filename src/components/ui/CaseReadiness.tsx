...
export function CaseReadiness({owner, action, dated, overdue=false, evidence,compact=false}:{owner:boolean;action:boolean;dated:boolean;overdue?:boolean;evidence?:number;compact?:boolean}) {
 const facts=[["Responsabil",owner],["Pas următor",action],["Termen",dated]] as const;
 return <span className="inline-flex flex-wrap items-center gap-2 text-micro font-normal text-[rgb(var(--text-muted))]">
  ...
  {!compact?<span>{overdue?"Termen depășit":!owner?"Fără responsabil":!action?"Pas de stabilit":!dated?"Termen de stabilit":"Pas planificat"}</span>:null}
