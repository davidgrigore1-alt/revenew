...
export function WorkflowPlaybooks() {
  return <section aria-labelledby="playbook-library-title" className="mt-6 overflow-x-auto">
    ...
    <div className={"overflow-x-auto divide-y divide-[rgb(var(--border))] " + patterns.workflowTable}>{workflowPlaybooks.map((playbook) => {
      const capability = workflowTriggerCapability(playbook.trigger);
      return <article key={playbook.id} className={patterns.playbookGrid + " px-4 py-4"}>
        ...
        <form action={createWorkflowFromPlaybook}><input type="hidden" name="playbookId" value={playbook.id}/><button className="focus-ring h-8 rounded-[8px] border border-[rgb(var(--border-strong))] px-3 text-xs font-semibold hover:bg-[rgb(var(--surface-hover))]">Creează draft</button></form>
      </article>;
    })}</div>
  </section>;
}
