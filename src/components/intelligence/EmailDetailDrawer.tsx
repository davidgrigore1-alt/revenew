...
import { readableEmailBody } from "@/lib/ui/email-reader";
            ...
            {thread.length > 1 ? <section className="mt-6">
              ...
              <div className="mt-3 divide-y divide-white/[0.07] border-y border-white/[0.08]">
                {thread.map((item) => <details key={item.id} open={item.id === messageId} className="group py-3">
                  ...
                  <p className="mt-3 whitespace-pre-wrap break-words pl-3 text-xs leading-6 text-[#aaa]">{readableEmailBody(item.body) || "Conținut text indisponibil."}</p>
                </details>)}
              </div>
            </section> : null}
