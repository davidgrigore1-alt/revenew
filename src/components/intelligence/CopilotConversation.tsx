    ...
    <div className={cn("grid min-h-0 gap-4", className)}>
      <form onSubmit={submit} className="product-work-surface grid gap-2 p-3.5 transition-[background-color,border-color] duration-normal ease-standard focus-within:border-[rgb(var(--intelligence)/0.72)] focus-within:bg-[rgb(var(--surface))]">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[rgb(var(--border-subtle))] px-2 pb-2">
          <p className="text-[0.6875rem] font-semibold text-[rgb(var(--text-muted))]">Context autorizat · {context.contextLabel ?? (context.pageType === "opportunity" ? "Oportunitatea curentă" : context.pageType === "company" ? "Compania curentă" : context.pageType === "dashboard" ? "Control Center" : "Întregul spațiu de lucru")}</p>
          <span className="text-[0.6875rem] text-[rgb(var(--text-faint))]">Enter pentru analiză · Shift+Enter pentru rând nou</span>
        </div>
      ...
      <div className="grid gap-5" aria-live="polite" aria-busy={loading}>
        ...
        {conversation.length === 0 && !loading ? (
          <section aria-labelledby="copilot-suggestions-title">
            ...
            <div className="grid gap-px overflow-hidden rounded-panel border border-[rgb(var(--border-subtle))] bg-[rgb(var(--border-subtle))] sm:grid-cols-2">
          ...
          <article key={item.id} className={cn("intelligence-reveal overflow-hidden rounded-panel border", index === 0 && !loading ? "border-[rgb(var(--border-strong))] bg-[rgb(var(--surface))]" : historyOpen ? "border-[rgb(var(--border))] bg-[rgb(var(--surface-subtle))]" : "hidden")} aria-labelledby={`${item.id}-answer`}>
            ...
            <div className="mt-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h4 className="text-xs font-semibold text-[rgb(var(--intelligence-strong))] dark:text-[rgb(var(--intelligence))]">Concluzie ReveNew</h4>
              ...
              <p className="mt-2 text-xs text-[rgb(var(--text-muted))]">Răspuns la {formatProductDateTime(item.answeredAt)} · Instantaneu al verificării. Întreabă din nou după modificări.</p>
