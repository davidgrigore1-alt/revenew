...
export function CommercialSignalImportWizard({ history }: { history: CommercialImportHistoryItem[] }) {
  const sampleCsv = "company,contact,email,phone,estimated_value,currency,last_interaction,context,source_reference\nExemplu Client SRL,Ana Exemplu,ana.exemplu@example.com,+40700000001,12500,RON,2026-06-15,Discuție veche fără follow-up pentru ofertă B2B.,demo-import-001";
  ...
  return <div className="grid gap-8">
    ...
    <section className="grid gap-4 border-b border-[rgb(var(--border))] pb-8">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
        <a href={sampleCsvHref} download="revenew-import-exemplu.csv" className="focus-ring font-semibold text-[rgb(var(--primary))]">Descarcă CSV exemplu</a>
        <p className="text-[rgb(var(--muted-foreground))]">Anteturile uzuale în română și engleză sunt sugerate automat; maparea trebuie confirmată.</p>
      </div>
    ...
    {rows.length ? <section className="grid gap-5 border-b border-[rgb(var(--border))] pb-8">
      <div><h2 className="text-lg font-semibold">Confirmă maparea coloanelor</h2><p className="mt-1 text-sm text-[rgb(var(--muted-foreground))]">ReveNew sugerează corespondențele după antet. Verifică fiecare câmp; coloanele nemapate nu sunt importate.</p></div>
...
function PreviewSection({ preview, onConfirm, pending }: { preview: CommercialImportPreview; onConfirm: () => void; pending: boolean }) {
  ...
  return <section className="grid gap-5 border-b border-[rgb(var(--border))] pb-8">
    <div><h2 className="text-lg font-semibold">Previzualizare validată</h2><p className="mt-1 text-sm text-[rgb(var(--muted-foreground))]">Valide = pot crea semnale. Respinse = nu vor fi salvate. Duplicate = sunt omise. Potrivirile probabile nu sunt comasate automat.</p></div>
