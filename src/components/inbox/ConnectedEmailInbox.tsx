...
export function ConnectedEmailInbox({ emails, responseWindowBusinessDays = 3, initialEmailId }: { emails: InboxEmail[]; responseWindowBusinessDays?: number; initialEmailId?: string }) {
  ...
  return (
    <section className="mt-6 border-t border-[rgb(var(--border))] pt-6" aria-labelledby="connected-email-inbox-title">
      <div className="flex items-end justify-between gap-4">
        <div className="flex items-start gap-3"><GmailMark /><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-[rgb(var(--primary))]">Context privat conectat</p><h2 id="connected-email-inbox-title" className="mt-1 text-lg font-semibold">Conversații Gmail recente</h2><p className="mt-1 text-sm text-[rgb(var(--text-muted))]">O coadă privată pentru context comercial; vizibilă numai utilizatorului care a autorizat conexiunea.</p></div></div>
        <span className="text-xs tabular-nums text-[rgb(var(--text-faint))]">{emails.length} mesaje</span>
      </div>
      <div className="mt-4 flex flex-wrap gap-1 border-b border-[rgb(var(--border))] pb-3" role="group" aria-label="Filtre conversații">
        {(Object.keys(viewLabels) as InboxView[]).map((item) => {
          const count = item === "all" ? classified.length : classified.filter((entry) => entry.state === item).length;
          return <button key={item} type="button" onClick={() => setView(item)} aria-pressed={view === item} className="focus-ring rounded-[var(--radius-control)] px-2.5 py-1.5 text-xs font-semibold text-[rgb(var(--text-muted))] hover:bg-[rgb(var(--surface-hover))] aria-pressed:bg-[rgb(var(--surface-elevated))] aria-pressed:text-[rgb(var(--foreground))]">{viewLabels[item]} <span className="ml-1 tabular-nums text-[rgb(var(--text-faint))]">{count}</span></button>;
        })}
      </div>
      <div className="divide-y divide-[rgb(var(--border))] border-b border-[rgb(var(--border))]">
        {visible.map(({ email, state }) => <button key={email.id} type="button" aria-haspopup="dialog" onClick={() => setSelected(email.id)} className="product-interactive-row focus-ring group grid w-full gap-3 px-3 py-3.5 text-left sm:grid-cols-[2.25rem_minmax(0,1fr)_auto] sm:items-start">
          <Monogram email={email} />
          <span className="min-w-0"><span className="flex min-w-0 items-center gap-2"><span className="truncate text-sm font-semibold">{email.senderName || email.senderEmail || (email.direction === "outbound" ? "Mesaj trimis" : "Expeditor neidentificat")}</span><span className={state === "reply_received" ? "status-pill status-pill-success" : state === "requires_action" ? "status-pill status-pill-warning" : "status-pill status-pill-neutral"}>{stateLabels[state]}</span></span><span className="mt-1 block truncate text-xs font-semibold text-[rgb(var(--text-secondary))]">{formatUserFacingText(email.subject || "Fără subiect", { stripUrls: true })}</span>{email.excerpt ? <span className="mt-1 block line-clamp-1 text-xs leading-5 text-[rgb(var(--text-muted))]">{formatUserFacingText(email.excerpt, { stripUrls: true })}</span> : null}</span>
        ...
        {!visible.length ? <p className="py-6 text-center text-sm text-[rgb(var(--text-muted))]">Nicio conversație în această stare.</p> : null}
      </div>
