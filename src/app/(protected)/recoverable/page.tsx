              ...
              <div className="app-scrollbar hidden max-w-full overflow-x-auto lg:block">
                <table className="w-full min-w-[980px] table-fixed border-collapse text-left text-sm">
                  ...
                  <tbody className="divide-y divide-[rgb(var(--border))]">
                    {queue.map(({ opportunity, state, assessment, primaryReason }) => {
              ...
              <div className="divide-y divide-[rgb(var(--border))] lg:hidden">
                {queue.map(({ opportunity, state, primaryReason }) => {
                  ...
                  return <article key={opportunity.id} className="p-4">
                    ...
                    <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-xs text-[rgb(var(--text-muted))]">Responsabil</dt><dd className="mt-1 font-medium">{state.nextAction?.ownerName ?? state.ownership.ownerName ?? "Neatribuit"}</dd></div><div><dt className="text-xs text-[rgb(var(--text-muted))]">Următoarea acțiune</dt><dd className="mt-1 font-medium">{state.nextAction?.title ?? "Lipsește"}</dd><p className="mt-1 text-xs text-[rgb(var(--text-muted))]">{state.nextAction?.dueAt ? formatDate(state.nextAction.dueAt) : "Fără termen"}</p></div></dl>
