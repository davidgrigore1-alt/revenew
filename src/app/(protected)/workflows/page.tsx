...
export default async function WorkflowsPage() {
  ...
  const activeCount = workspace.workflows.filter((workflow) => workflow.status === "active").length;
  const attentionCount = workspace.runs.filter((run) => run.status === "failed").length;
  const draftCount = workspace.workflows.filter((workflow) => workflow.status === "draft" || workflow.status === "paused").length;
  ...
  >
    ...
    <section aria-labelledby="workflow-list-title" className="overflow-hidden border-y border-[rgb(var(--border))]">
      ...
      {workspace.workflows.length ? <div className="overflow-x-auto">
        <div className={patterns.workflowTable}>
          <div className={patterns.workflowGrid + " border-t border-[rgb(var(--border))] px-4 py-2 text-[11px] font-medium text-[rgb(var(--text-muted))]"}>
            <span>Nume / acțiuni</span><span>Stare</span><span>Declanșator / control</span><span>Ultima evaluare</span><span>Rezultat</span><span className="text-left">Acțiuni</span>
          </div>
