...
export default async function FollowUpStudioPage({params}:{params:{id:string}}){
 ...
 return <PageShell eyebrow="Follow-up Studio" title={normalized.subject||"Draft comercial"} description="Revizuire și aprobare umană înaintea oricărei utilizări externe."><FollowUpStudio initialDraft={{id:document.id,opportunityId:opportunity.id,opportunityTitle:opportunity.title,subject:normalized.subject,body:normalized.body,status:document.status,generationMode:document.generation_mode??"local_fallback",recipientEmail:opportunity.contact_email??undefined,contactName:opportunity.contact_name??undefined,reason:opportunity.recommended_action??opportunity.summary??undefined,dueDate:action?.due_at??undefined,createdAt:document.created_at??undefined}} timeline={timeline}/></PageShell>
}