...
export default async function FollowUpStudioPage(props:{params: Promise<{id:string}>}) {
 ...
 return <PageShell eyebrow="Studio de follow-up" title={normalized.subject||"Draft comercial"} description="Revizuire, confirmare umană și trimitere controlată pe server."><FollowUpStudio initialDraft={{id:document.id,opportunityId:opportunity.id,opportunityTitle:opportunity.title,subject:normalized.subject,body:normalized.body,status:document.status,generationMode:document.generation_mode??"local_fallback",recipientEmail:opportunity.contact_email??undefined,contactName:opportunity.contact_name??undefined,reason:opportunity.recommended_action??opportunity.summary??undefined,dueDate:action?.due_at??undefined}} timeline={timeline} initialReadiness={readiness}/></PageShell>
}
