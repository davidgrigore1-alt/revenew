  ...
  return (
    <main className="mx-auto grid w-full max-w-7xl gap-6 px-4 py-8 pb-24 sm:px-6 lg:px-8 xl:pb-8">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Valoare estimată recuperată (RON)" value={formatCurrency(wonRonEstimate, "RON")} detail="Estimare doar în RON, nu venit confirmat contabil." />
      </div>
          ...
          {lost.length > 0 ? (
            lost.map((opportunity) => (
              <div key={opportunity.id} className="rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--surface-elevated))] p-4">
                ...
                <p className="mt-1 text-sm text-[rgb(var(--muted-foreground))]">Valoare estimată pierdută: {formatCurrency(opportunity.estimatedValueHigh, opportunity.currency ?? "RON")}</p>
              </div>
            ))
          ) : (
