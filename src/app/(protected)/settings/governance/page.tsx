...
export default async function GovernancePage() {
  ...
  if (!canAccessGovernance) {
    return (
      <PageShell
        eyebrow="Setări spațiu de lucru"
        title="Echipă și guvernanță"
        description="Administrarea accesului este disponibilă numai rolurilor autorizate."
      >
        <ForbiddenState
          title="Acces restricționat"
          description="Rolul tău nu include administrarea echipei, politicilor sau aprobărilor acestui spațiu de lucru."
        />
      </PageShell>
    );
  }
  ...
  return (
    <PageShell eyebrow="Setări spațiu de lucru" title="Echipă și guvernanță" description="Acces, atribuiri, aprobări și audit pentru operarea sigură a echipei." actions={<Link href="/settings" className="focus-ring inline-flex min-h-11 items-center rounded-button border border-[rgb(var(--border))] px-4 text-sm font-semibold hover:bg-[rgb(var(--surface-muted))]">Înapoi la setări</Link>}>
