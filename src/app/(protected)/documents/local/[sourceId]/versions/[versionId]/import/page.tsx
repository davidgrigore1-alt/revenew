...
export default async function ImportSavedDocument({params}:{params:Promise<{sourceId:string;versionId:string}>}) {
 ...
 return <PageShell wide eyebrow="Document păstrat" title="Importă date în ReveNew" description="Importul este opțional. Originalul rămâne păstrat dacă închizi această pagină."><Link className="focus-ring mb-6 inline-block text-sm underline" href={href}>← Înapoi la document</Link><DocumentCsvImport initialSource={{csv:{headers:doc.version.headers??[],rows:doc.segments.map(s=>s.cells),delimiter:","},name:doc.version.original_filename,bytes:doc.version.byte_size??0,href}}/></PageShell>;
}
