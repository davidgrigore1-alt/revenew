...
export async function GET(request: Request) {
  ...
  if (!isGoogleWorkspaceConfigured()) {
    return NextResponse.redirect(browserSafeRedirectUrl(request.url, "/apps?google=not-configured", "/apps?google=not-configured"));
  }
  ...
  if (purpose === "drive" && !connection) {
    return NextResponse.redirect(browserSafeRedirectUrl(request.url, "/apps?google=connection-required", "/apps?google=connection-required"));
  }
