...
const paidPlans = previewPlans.map((plan) => {
  ...
  return {
    ...
    items: audit
      ? ["maparea oportunităților și a valorii urmărite", "revizuirea responsabilității și a follow-up-ului", "recomandări și pași următori documentați"]
...
export default async function AccessPage(props: { searchParams?: Promise<{ reason?: string }> }) {
  ...
  return (
    <main className="account-light-theme min-h-screen overflow-hidden bg-[rgb(var(--background))] text-[rgb(var(--foreground))]">
      <header className="sticky top-0 z-40 border-b border-[rgb(var(--border))] bg-[rgb(var(--background)/0.92)] backdrop-blur-xl">
        <div className="mx-auto flex min-h-16 max-w-[1200px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <Logo />
          <div className="flex items-center gap-1 sm:gap-3">
            <Link href="/billing" className="focus-ring inline-flex min-h-10 items-center rounded-button px-3 text-sm font-semibold text-[rgb(var(--text-secondary))] transition-colors duration-fast hover:bg-[rgb(var(--surface-muted))] hover:text-[rgb(var(--foreground))]">Cont<span className="hidden sm:inline"> și facturare</span></Link>
      ...
      <section className="relative border-b border-[#dde3dd]">
        <div aria-hidden="true" className="absolute inset-0 bg-[radial-gradient(circle_at_80%_18%,rgb(var(--brand-500)/0.12),transparent_34%),linear-gradient(180deg,rgb(var(--surface-elevated))_0%,rgb(var(--background))_100%)]" />
        <div className="relative mx-auto grid max-w-[1200px] gap-10 px-4 py-12 sm:px-6 sm:py-16 lg:grid-cols-[minmax(0,1.25fr)_minmax(340px,0.75fr)] lg:items-center lg:px-8 lg:py-20">
          <div className="max-w-3xl">
            ...
            <p className="mt-6 max-w-2xl text-base leading-7 text-[#3f4a45] sm:text-lg">
              Contul și spațiul de lucru ReveNew sunt create. Accesul operațional devine disponibil după confirmarea planului sau a implementării comerciale, fără să schimbe controlul uman asupra deciziilor.
            </p>
          ...
          <section aria-labelledby="access-status-heading" className="rounded-[1.25rem] border border-[#c8d1c8] bg-white p-5 shadow-[0_28px_80px_rgba(15,23,42,0.10)] sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[rgb(var(--primary))]">Stare cont</p>
                <h2 id="access-status-heading" className="mt-2 text-xl font-semibold tracking-[-0.02em]">Acces explicat, fără blocaje tehnice</h2>
              </div>
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[rgb(var(--brand-100))] text-[rgb(var(--primary))]"><CheckBadgeIcon className="h-5 w-5" aria-hidden="true" /></span>
            </div>
            <dl className="mt-6 divide-y divide-[#dde3dd] border-y border-[#dde3dd]">
              <div className="grid gap-1 py-4 sm:grid-cols-[120px_1fr]"><dt className="text-sm text-[#6b756f]">Spațiu de lucru</dt><dd className="font-semibold">{context.currentBusiness.business.name}</dd></div>
      ...
      <section id="planuri" className="scroll-mt-24 border-b border-[#dde3dd] bg-[#f2f4f1]">
        <div className="mx-auto max-w-[1200px] px-4 py-14 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[rgb(var(--primary))]">Opțiuni comerciale</p>
        ...
        <div className="mx-auto max-w-[1200px] px-4 py-14 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
          <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
            <div className="max-w-xl">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[rgb(var(--primary))]">După confirmare</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-[-0.035em]">Un proces controlat, nu un simplu checkout</h2>
              <p className="mt-4 text-base leading-7 text-[#3f4a45]">Activarea stabilește accesul comercial. Datele, responsabilitatea și pașii următori rămân în spațiul de lucru al companiei și sunt operate de utilizatori autorizați.</p>
            </div>
            <ol className="grid gap-4 sm:grid-cols-3">
              {[
                ["01", "Confirmare", "Planul sau implementarea este confirmată prin fluxul existent."],
                ["02", "Configurare", "Spațiul de lucru este pregătit pentru datele și procesul comercial al echipei."],
                ["03", "Control operațional", "Echipa prioritizează oportunitățile, responsabilitatea și follow-up-ul din Control Center."]
              ].map(([number, title, description]) => (
                <li key={number} className="rounded-panel border border-[#dde3dd] bg-[#fafaf8] p-5">
                  <span className="text-xs font-semibold text-[rgb(var(--primary))]">{number}</span>
      ...
      <section className="border-y border-[#dde3dd] bg-[#f7f8f6]">
        <div className="mx-auto max-w-[1200px] px-4 py-12 sm:px-6 lg:px-8">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <TrustItem icon={<CircleStackIcon className="h-5 w-5" aria-hidden="true" />} title="Spațiu de lucru izolat" description="Datele sunt încărcate în contextul companiei autorizate." />
            ...
            <TrustItem icon={<UserGroupIcon className="h-5 w-5" aria-hidden="true" />} title="Responsabilitate clară" description="Responsabilitatea și următorul pas rămân vizibile." />
      ...
      <section className="bg-white">
        <div className="mx-auto grid max-w-[1200px] gap-10 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[0.7fr_1.3fr] lg:px-8 lg:py-20">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[rgb(var(--primary))]">Întrebări utile</p>
            ...
            {[
              ...
              <details key={question} className="group">
                <summary className="focus-ring flex min-h-14 cursor-pointer list-none items-center justify-between gap-4 rounded-control py-4 font-semibold marker:hidden">
                  <span>{question}</span><span aria-hidden="true" className="text-xl font-normal text-[rgb(var(--primary))] transition-transform duration-fast group-open:rotate-45">+</span>
                </summary>
      ...
      <section className="border-t border-[#2d2e2f] bg-[#111213] text-white">
        <div className="mx-auto flex max-w-[1200px] flex-col gap-6 px-4 py-12 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div className="max-w-2xl">
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#e6c84a]">Următorul pas</p>
