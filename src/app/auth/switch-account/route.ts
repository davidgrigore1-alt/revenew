...
import { authPath, browserSafeRedirectUrl, safeInternalRedirect, sanitizeAuthIntent } from "@/lib/auth/redirects";
...
export async function GET(request: NextRequest) {
  ...
  const response = NextResponse.redirect(browserSafeRedirectUrl(request.url, next.includes("?") ? `${next}&reason=account_switched` : `${next}?reason=account_switched`, "/login"));
