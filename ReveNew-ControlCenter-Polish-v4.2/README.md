# ReveNew — Control Center Polish V4.2
...
`C:\Projects\ReveNew\ReveNew-ControlCenter-Polish-v4.2`

Then from CMD:

```bat
cd /d C:\Projects\ReveNew
powershell -NoProfile -ExecutionPolicy Bypass -File ".\ReveNew-ControlCenter-Polish-v4.2\apply.ps1"
```
...
```bat
powershell -NoProfile -ExecutionPolicy Bypass -File ".\ReveNew-ControlCenter-Polish-v4.2\verify.ps1" -Build
```
...
V4.2 corrects the PowerShell newline insertion used for `contextDescription`.
It also validates the transformed dashboard source before backup/write and refuses
to write if a literal PowerShell `` `n `` token appears where TypeScript expects
a real newline.

If V4.1 reached typecheck and failed with `Property assignment expected`, run
the V4.1 `rollback.ps1` before applying V4.2.
