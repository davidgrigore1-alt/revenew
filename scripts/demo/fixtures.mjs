...
export function buildFixtures(profileId, now = new Date()) {
  ...
  const recommendedSignalActions = [
    "Confirmă cerința, valoarea, moneda și termenul înainte de a pregăti următorul pas comercial.",
    "Confirmă cerința de extindere, bugetul și termenul înainte de a pregăti o ofertă revizuită.",
    "Verifică istoricul și confirmă o revenire cu responsabil și termen clar.",
    "Confirmă data contractuală, condițiile de reînnoire și responsabilul comercial.",
    "Evaluează impactul, desemnează un responsabil și stabilește intervenția umană prioritară.",
    "Confirmă motivul pierderii și păstrează decizia în contextul oportunității existente.",
    "Confirmă sursa recomandării, persoana de contact și relevanța comercială înainte de calificare.",
    "Completează contactul, valoarea și următorul pas înainte de calificare.",
    "Verifică rezultatul deja înregistrat și păstrează semnalul ca element auditabil.",
    "Leagă semnalul de oportunitatea existentă și confirmă intern următoarea acțiune, responsabilul și termenul."
  ];
  ...
  const signalEvents = [...signals.map((signal, index) => ({
    ...
    description: "Recomandarea locală a fost editată de utilizator înainte de decizie.",
    metadata: { demo: true, external_action: false, preparation_mode: "deterministic_fallback", original_recommended_action: recommendedSignalActions[1], final_recommended_action: signals[1].recommended_action, edited_fields: ["next_action"] },
    created_by_profile_id: profileId, created_at: iso(-1)
  }, {
    id: signalEventIds[11], business_id: DEMO.businessId, signal_id: signalIds[8], event_type: "recommendation_feedback_recorded",
    description: "Recomandare acceptată fără modificări și aplicată intern.",
    metadata: { demo: true, external_action: false, preparation_mode: "deterministic_fallback", feedback_state: "accepted_as_is", original_recommended_action: signals[8].recommended_action, final_approved_action: signals[8].recommended_action, edited: false, edited_fields: [], conversion_type: "new_opportunity", opportunity_id: opportunityIds[8] },
    created_by_profile_id: profileId, created_at: iso(-3)
  }];
