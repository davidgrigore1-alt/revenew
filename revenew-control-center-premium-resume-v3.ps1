...
$backupDir = Join-Path $Root ".manual-ui-backups\control-center-premium-resume-v3-$stamp"
...
function Replace-Exact-Or-Skip {
    param(
        [string]$Text,
        [string]$Old,
        [string]$New,
...
function Replace-Regex-One-Or-Skip {
    ...
    if ($matches.Count -ne 1) {
        throw "SAFE STOP: expected exactly one regex match for '$Label', found $($matches.Count)"
    }
    Write-Host "OK  $Label" -ForegroundColor Green
    return [regex]::Replace($Text, $Pattern, $Replacement, 1)
}

function Insert-After-Exact-Or-Skip {
    param(
        [string]$Text,
        [string]$Anchor,
        [string]$Insert,
...
# 1) Premium disclosure: intervention preparation
$old = @'
...
$pageText = Replace-Exact-Or-Skip `
    $pageText $old $new `
    "Revizuiește intervențiile pregătite înainte de orice aplicare." `
    "Disclosure: intervention preparation"

# 2) Premium disclosure: secondary signals
$old = @'
            <summary className="control-center-disclosure-summary focus-ring flex cursor-pointer list-none items-center justify-between gap-4 marker:hidden">
...
$pageText = Replace-Exact-Or-Skip `
    $pageText $old $new `
    "Semnale secundare care pot schimba ordinea de intervenție." `
    "Disclosure: secondary signals"

# 3) Analyst subtitle — robust across whitespace / line endings
$pattern = 'Întreabă ce s-a schimbat,\s*de ce contează\s*(?:\r?\n)?\s*sau pregătește următorul pas\.'
$replacement = 'Verifică schimbările, blocajele și următorul pas sigur.'

$pageText = Replace-Regex-One-Or-Skip `
    $pageText $pattern $replacement `
    "Verifică schimbările, blocajele și următorul pas sigur." `
    "Analyst disclosure subtitle"

# 4) Chevron optical centering
$old = @'
...
$new = @'
  ...
  transform-origin: 50% 50%;
'@
$cssText = Replace-Exact-Or-Skip `
    $cssText $old $new `
    "transform: translateY(-1px) rotate(45deg);" `
    "Chevron closed optical centering"
...
$cssText = Replace-Exact-Or-Skip `
    $cssText $old $new `
    "transform: translateY(1px) rotate(225deg);" `
    "Chevron open optical centering"

# 5) Analyst surface composition
$old = @'
...
$cssText = Replace-Exact-Or-Skip `
    $cssText $old $new `
    "width: min(100%, 68rem);" `
    "Analyst surface composition"

# 6) Connected context full-width
$old = @'
...
$cssText = Replace-Exact-Or-Skip `
    $cssText $old $new `
    "max-width: none;" `
    "Connected context full-width grid"

# 7) Light-only premium refinements
$anchor = @'
...
$cssText = Insert-After-Exact-Or-Skip `
    $cssText $anchor $insert `
    "Control Center premium light-mode refinements" `
    "Light-only premium depth refinements"

# Save only after every safety check above has succeeded.
...
Write-Host ""
Write-Host "DONE - Control Center premium resume V3 installed." -ForegroundColor Green
Write-Host "Dark-mode palette was not redefined." -ForegroundColor Green
Write-Host "Refresh with Ctrl+Shift+R and inspect light + dark mode." -ForegroundColor Yellow
