...
import { evidenceSourceLabels } from "@/lib/evidence-reference";
...
function CaseDetail({ item,impactHref }: { item: ExecutionCase;impactHref?:string }) {
  return <article aria-labelledby="selected-execution-title" className="min-w-0 px-5 py-5 xl:px-6">
    <header className="border-b border-[rgb(var(--border))] pb-4">
      <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[rgb(var(--primary))]">Caz selectat</p>
    ...
    <section className="py-4" aria-labelledby="execution-why-now">
      ...
      <details className="mt-3"><summary className="focus-ring cursor-pointer text-[11px] text-[rgb(var(--text-muted))]">Cum este stabilită prioritatea</summary>
    ...
    <section className="border-y border-[rgb(var(--border))] py-4" aria-labelledby="execution-next-action">
      <div className="flex items-baseline justify-between gap-3"><h3 id="execution-next-action" className={sectionTitle}>Următorul pas</h3><span className="text-[11px] text-[rgb(var(--text-muted))]">Control uman</span></div>
...
export function ExecutionControlCenter({ model,impactLinks={} }: { model: ExecutionControlCenterModel;impactLinks?:Record<string,string> }) {
  ...
  return <section aria-labelledby="execution-center-title" className="pt-6">
    <header className="flex flex-wrap items-end justify-between gap-4">
      <div><h1 id="execution-center-title" className="text-xl font-semibold tracking-tight">Control Center</h1><p className="mt-1 text-sm text-[rgb(var(--text-muted))]">Situațiile comerciale care necesită intervenție.</p></div>
    ...
    <div role="group" aria-label="Filtrează situațiile" className="flex flex-wrap gap-1 border-b border-[rgb(var(--border))] py-3">
      {filters.map((option) => <button key={option.id} type="button" aria-pressed={filter === option.id} onClick={() => setFilter(option.id)}
        className={cn("focus-ring rounded-control px-3 py-1.5 text-xs transition-colors duration-fast", filter === option.id ? "bg-[rgb(var(--surface-elevated))] font-semibold text-[rgb(var(--foreground))]" : "text-[rgb(var(--text-muted))] hover:bg-[rgb(var(--surface))]")}>{option.label}<span className="ml-1.5 text-[11px] tabular-nums text-[rgb(var(--text-muted))]">{model.cases.filter((item) => matches(item, option.id)).length}</span></button>)}
    </div>
    {visible.length ? <div className="grid min-w-0 border-b border-[rgb(var(--border))] lg:grid-cols-[minmax(0,1.1fr)_minmax(350px,1fr)]">
      <div className="min-w-0 lg:max-h-[min(880px,78vh)] lg:overflow-y-auto">
        <p className="px-3 py-3 text-[11px] text-[rgb(var(--text-muted))]">Ordine de intervenție · selectează un caz</p>
        <ul aria-label="Coada de execuție comercială" className="divide-y divide-[rgb(var(--border))]">{visible.map((item) => <li key={item.id}>
          <button type="button" aria-pressed={selected?.id === item.id} aria-controls="execution-case-detail" onClick={() => setSelectedId(item.id)}
            className={cn("focus-ring relative flex w-full min-w-0 gap-3 border-l-2 px-3 py-3.5 text-left transition-colors duration-fast", selected?.id === item.id ? "border-[rgb(var(--primary))] bg-[rgb(var(--primary-soft))]" : "border-transparent hover:bg-[rgb(var(--surface))]")}>
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-[rgb(var(--text-muted))]"><span>{item.owner.name}</span>
                {item.overdue ? <span>{item.overdueDays ? "Restant · " + item.overdueDays + (item.overdueDays === 1 ? " zi" : " zile") : "Termen depășit"}</span> : null}
                <span>{Array.from(new Set(item.evidence.map((source) => evidenceSourceLabels[source.sourceType]))).slice(0, 3).join(" · ")}</span>
              </span>
            </span>
      ...
      <div key={selected?.id} id="execution-case-detail" className="min-w-0 border-t border-[rgb(var(--border))] bg-[rgb(var(--surface))] lg:max-h-[min(880px,78vh)] lg:overflow-y-auto lg:border-l lg:border-t-0">
