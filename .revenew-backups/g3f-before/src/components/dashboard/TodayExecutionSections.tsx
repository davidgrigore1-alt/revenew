...
export function TodayExecutionSections({ queue }: { queue: WorkspaceDecisionQueue }) {
  ...
  return <section aria-labelledby="today-operating-title">
    ...
    <div className="grid gap-x-8 xl:grid-cols-2">
      {(Object.keys(sectionCopy) as SectionKey[]).map((key) => <section key={key} className="border-b border-[rgb(var(--border))] py-5" aria-labelledby={"today-" + key}>
        ...
        {groups[key].length ? <ol className="mt-3 divide-y divide-[rgb(var(--border))] border-y border-[rgb(var(--border))]">{groups[key].slice(0, 5).map((item) => <Row key={item.id} item={item} />)}</ol> : <p className="mt-3 py-3 text-sm text-[rgb(var(--text-muted))]">{sectionCopy[key].empty}</p>}
      </section>)}
    </div>
