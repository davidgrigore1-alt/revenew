...
function IntegrationCard({
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex min-h-[22px] items-start gap-2 overflow-hidden" aria-label={provider ? "Servicii Google; Drive, Docs, Sheets și Meet sunt planificate" : "Capabilități planificate"}>
        {item.capabilities.slice(0, 3).map((capability) => (
          <span key={capability} title={provider && capability === "Drive" ? "Drive · planificat" : capability} className="inline-flex h-[22px] min-w-0 items-center text-[11px] text-[rgb(var(--text-secondary))]">
            <span className="truncate">{capability}{provider && capability === "Drive" ? " · planificat" : ""}</span>
          </span>
        ))}
