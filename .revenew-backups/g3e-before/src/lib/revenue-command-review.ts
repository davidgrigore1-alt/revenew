...
export function verifyReviewTicket(token:string,secret:string,actor:{businessId:string;actorId:string;scope:string},now=new Date()):ReviewTicket{
 ...
 if(ticket.businessId!==actor.businessId||ticket.actorId!==actor.actorId||ticket.scope!==actor.scope||!/^\w{8}-\w{4}-\w{4}-\w{4}-\w{12}$/.test(ticket.requestId))throw new Error("review_forbidden");
