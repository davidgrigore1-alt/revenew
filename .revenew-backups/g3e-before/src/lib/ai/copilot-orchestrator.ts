...
export function planCopilotRequest(request: CopilotRequest) {
  ...
  if (/intalnir|calendar|meeting|maine|saptamana aceasta/.test(query)) return { name: "get_external_context", args: { view: /maine/.test(query) ? "meetings_tomorrow" : /azi|astazi/.test(query) ? "meetings_today" : /saptamana/.test(query) ? "meetings_week" : "recent_interactions" } };
...
export async function runCopilot(request: CopilotRequest, provider: CopilotProvider = getCopilotProvider()): Promise<CopilotRunResult> {
  ...
  if (!prohibitedRequest(request.question) && !unsupportedInference(request.question)
    ...
    && /cine (?:este|e) responsabil|care este urmatorul pas|contrazic|neconcord|asocierea document|difer.*(?:crm|dovez|ofert)|(?:ofert|dovez).*(?:urmator|pas|pipeline)|document.*(?:deadline|termen)|adevar comercial|context verific|de ce.*(?:risc|blocat)|ce (?:trebuie facut|informatii lipsesc)|ce s-a schimbat.*(?:7 zile|document)/.test(truthQuestion)) {
