...
export async function revenueCommandAnswer(request:CopilotRequest):Promise<CopilotAnswer> {
 ...
 try {
  ...
  const evidence=chosen.flatMap(d=>d.evidence).slice(0,8).map(e=>({
   sourceId:e.sourceType+":"+e.sourceId,label:e.title,sourceType:"Oportunitate" as const,route:e.entityHref??null,
  ...
  if(/schimbat|intamplat/.test(q)){
   ...
   evidence.splice(0,evidence.length,...changes.map(c=>({sourceId:c.id,label:c.label,sourceType:"Oportunitate" as const,route:c.evidence.entityHref??null,fact:c.detail,observedAt:c.at,claimType:"fact" as const})));
