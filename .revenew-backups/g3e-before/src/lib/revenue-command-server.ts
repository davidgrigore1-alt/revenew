...
export const getRevenueCommand=cache(async(range?:string)=>{
 ...
 ids.length?client.from("opportunity_actions").select("id,business_id,opportunity_id,type,title,status,due_at,priority,assigned_to_profile_id,created_at,updated_at").eq("business_id",actor.businessId).in("opportunity_id",ids).eq("status","pending").order("due_at",{ascending:true,nullsFirst:false}).limit(1001):empty,
 ids.length?client.from("opportunity_events").select("id,business_id,opportunity_id,event_type,label,metadata,actor_profile_id,occurred_at").eq("business_id",actor.businessId).in("opportunity_id",ids).gte("occurred_at",oldest).lte("occurred_at",now.toISOString()).in("event_type",Object.keys(materialEventLabels)).order("occurred_at",{ascending:false}).limit(401):empty,
 ...
 client.from("commercial_signals").select("id,business_id,title,status,review_status,detected_from_opportunity_id,converted_opportunity_id,review_due_at,created_at,updated_at").eq("business_id",actor.businessId).or("detected_from_opportunity_id.in.("+ids.join(",")+"),converted_opportunity_id.in.("+ids.join(",")+")").order("updated_at",{ascending:false}).limit(241),
 ids.length?client.from("opportunity_documents").select("id,opportunity_id,title,document_type,status,created_at,ready_at,sent_at").eq("business_id",actor.businessId).in("opportunity_id",ids).in("status",["approved","ready_to_send"]).limit(161):empty,
 ids.length?client.from("commercial_workflow_runs").select("id,target_id,completed_at,prepared_action_plan_ids").eq("business_id",actor.businessId).in("target_id",ids).eq("is_test_run",false).gte("completed_at",oldest).order("completed_at",{ascending:false}).limit(81):empty,
 ...
 for(const e of signalEvents.data??[]){
  ...
  const completed=["approved","approval_granted","converted"].includes(e.event_type);
 ...
 const currentComplete=![actions,events,contacts,signals,documents].some(r=>r.error)&&
 ...
 const changesComplete=!events.error&&!runs.error&&!signalEvents.error&&(events.data?.length??0)<=400&&(runs.data?.length??0)<=80&&(signalEvents.data?.length??0)<=400;
 ...
 const reviewToken=!checkpoint.error&&currentComplete&&changesComplete&&secret?signReviewTicket({businessId:actor.businessId,actorId:actor.actorId,scope:actor.scope,through:model.generatedAt,requestId:randomUUID()},secret):null;
 return {...model,reviewToken,checkpointAvailable:!checkpoint.error,scope:actor.scope};
});
