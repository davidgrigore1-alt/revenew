...
export type CommandDecision={id:string;state:OpportunityCommercialState;reason:string;rule:string;rank:number;action:{label:string;href:string};evidence:EvidenceReference[]};
...
export function materialChanges(opportunities:Opportunity[],businessId:string,names:Record<string,string>):CommandChange[]{
 return opportunities.filter(o=>o.businessId===businessId).flatMap(o=>o.timeline.flatMap(event=>{
  ...
  const detail=ownerEvent?ownerName(m.previous_owner_profile_id)+" → "+ownerName(m.owner_profile_id)
   :event.type==="action_postponed"&&typeof m.due_at==="string"?"Termen înregistrat: "+m.due_at
...
export function compareCommandDecisions(a:CommandDecision,b:CommandDecision){
 ...
 const urgency=time(a.state)-time(b.state);if(Number.isFinite(urgency)&&urgency)return urgency;
...
export function assembleRevenueCommand(input:{
 ...
 const opportunities=input.opportunities.filter(o=>o.businessId===input.businessId);
 ...
 if(input.currentComplete!==false)for(const state of states){
  ...
  decisions.push({id:state.opportunityId,state,reason:reason.explanation+(reason.code==="pending_approval"&&state.resolvedSinceDetection.length?" Responsabilitatea a fost clarificată; aprobarea rămâne deschisă.":""),
 ...
 const periodChanges=Array.from(new Map(baseChanges.filter(c=>c.at>=period.from&&c.at<=period.to).map(c=>[c.id,c])).values()).sort((a,b)=>b.at.localeCompare(a.at)||a.id.localeCompare(b.id));
 const since=periodChanges.filter(c=>!input.checkpoint||c.at>input.checkpoint);
 ...
 return {generatedAt:period.to,period,checkpoint:input.checkpoint??null,decisions:decisions.slice(0,COMMAND_LIMITS.decisions),decisionCount:decisions.length,
  ...
  detected:input.currentComplete===false?null:sumImpactMoney(decisions.map(d=>({amount:d.state.financial.estimatedValue,currency:d.state.financial.currency}))),
  ...
  limited:!!input.limited,changesComplete:input.changesComplete!==false,historyClipped:!!input.checkpoint&&input.checkpoint<period.from};
}
