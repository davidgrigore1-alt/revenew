...
export default async function DashboardPage({searchParams={}}:{searchParams?:{view?:string;range?:string}}) {
  try {
    if(searchParams.view==="executive"){
      const model=await getRevenueCommand(searchParams.range);
      return <div className="mx-auto w-full max-w-[1600px] px-4 pb-12 sm:px-6 lg:px-8"><ControlCenterViews active="executive"/><RevenueCommandBrief model={model}/></div>;
    }
    const [summary, currentProfile, googleState, communicationsByOpportunityId] = await Promise.all([getRevenueWorkspaceSummary(), getCurrentProfile(), getGoogleWorkspacePublicState(), getOwnedCommunicationIndex()]);
    ...
    return (
      <div className="mx-auto w-full max-w-[1600px] px-4 pb-24 sm:px-6 lg:px-8 lg:pb-12">
        ...
        <ExecutionControlCenter model={executionCenter} impactLinks={impactLinks} />
