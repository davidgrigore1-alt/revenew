...
export async function runCopilot(request: CopilotRequest, provider: CopilotProvider = getCopilotProvider()): Promise<CopilotRunResult> {
  ...
  if (!prohibitedRequest(request.question) && !unsupportedInference(request.question)
    ...
    && /contrazic|neconcord|asocierea document|difer.*(?:crm|dovez|ofert)|(?:ofert|dovez).*(?:urmator|pas|pipeline)|document.*(?:deadline|termen)|adevar comercial|context verific|de ce.*(?:risc|blocat)|ce (?:trebuie facut|informatii lipsesc)|ce s-a schimbat.*(?:7 zile|document)/.test(truthQuestion)) {
