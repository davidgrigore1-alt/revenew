...
function harness({denied=false,queryError=false,rpcError=false,known=[]}={}) {
  ...
  const mocks={
    ...
    '@/lib/authz/require-permission':{async requirePermission(permission){calls.push({permission});if(denied)throw new Error('denied');}},
...
test('Documents uses the existing server authority and tenant-scoped preview without writes',async()=>{
  ...
  assert.equal(h.calls.filter(call=>call.name==='import_commercial_signal_batch').length,0);
});
...
test('confirmation forwards only explicit selections and retains durable replay identity',async()=>{
  ...
  const rpc=h.calls.find(call=>call.name==='import_commercial_signal_batch');assert.equal(rpc.args.target_business_id,'fixture-business');assert.equal(rpc.args.accepted_rows.length,1);assert.equal(rpc.args.accepted_rows[0].title,'Ofertă A');
